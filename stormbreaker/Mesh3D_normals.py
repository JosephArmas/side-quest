...
class Mesh3D:
    ...
    def load_textured_obj(obj_file, texture) -> "Mesh3D":
        ...
        for i in range(0, len(faces), 9):
            ...
            v1_texindexes = vertex_textures.setdefault(
                v1_index, {(texindex_v1, nrmindex_v1): v1_index}
            )
            v2_texindexes = vertex_textures.setdefault(
                v2_index, {(texindex_v2, nrmindex_v2): v2_index}
            )
            v3_texindexes = vertex_textures.setdefault(
                v3_index, {(texindex_v3, nrmindex_v3): v3_index}
            )
        ...
        # Og w/o assimp library
        # return Mesh3D(
        #     array(vertex_buffer, "float32"), array(face_buffer, "uint32"), texture
        # )
        return Mesh3D(
            np.array(vertex_buffer, "float32"), np.array(face_buffer, "uint32"), texture
        )
    ...
    def load_assimp_mesh(mesh_obj, texture):
        ...
        for i in range(0, len(vertexbuffer), 8):
            ...
            # new
            # vertexbuffer[i + 6] = texes[vnum % len(texes)][0]
            # vertexbuffer[i + 7] = texes[vnum % len(texes)][1]
